...
pub fn downsample_box_filter(pixels: &[u8], width: u32, height: u32) -> (Vec<u8>, u32, u32) {
    ...
    for out_y in 0..out_height {
        ...
        for out_x in 0..out_width {
            ...
            let out_offset = (out_y * out_width + out_x) * 4;
            for channel in 0..4 {
...
fn texel_at(pixels: &[u8], width: usize, x: usize, y: usize) -> [u8; 4] {
...
pub fn build_mip_chain(
    ...
    for _ in 0..max_level {
        let (prev_pixels, prev_width, prev_height) =
            chain.last().expect("chain always has a base level");
...
pub fn classify_alpha_shape(
    ...
    for y in 0..height {
        let row_start = src_offset + y * width * 4;
    ...
    if saw_transparent && saw_opaque {
        AlphaShape::PunchThrough
    } else if saw_transparent {
        AlphaShape::PunchThrough
    } else {
        AlphaShape::FullyOpaque
    }
}
        ...
        for bx in 0..DCT_SIZE {
            ...
            for y in start_y..end_y.min(height) {
                for x in start_x..end_x.min(width) {
                    let offset = src_offset + (y * width + x) * 4;
...
// Total-order comparator: DCT coefficients are IEEE-754 f64 and can carry NaN
// if a source texture has garbage/uninitialized alpha or an upstream mixin
// hands us malformed pixel data. The previous partial_cmp().unwrap() panics
// on NaN, and this crate builds with panic = "abort" (required -- unwinding
// across a JNI boundary is UB), so a single NaN here previously took down the
// entire JVM process, not just this call. total_cmp defines a total order
// over every f64 bit pattern including NaN, so there is no panic path left.
...
fn group_by_similarity(
    ...
    for i in 0..count {
        by_tint.get_mut(&tinted[i]).unwrap().push(i);
    }
    ...
    for i in 0..count {
        ...
        while let Some(current) = frontier.pop() {
            let bucket = &by_tint[&tinted[current]];
            for &j in bucket {
                if visited[j] {
                    continue;
                }
                if (fingerprints[current] ^ fingerprints[j]).count_ones() <= max_hamming_distance {
                    visited[j] = true;
                    members.push(j);
                    frontier.push(j);
                }
...
fn assemble_atlas(
    ...
    for sprite in sprites {
        ...
        for y in 0..sprite_height {
            let src_row_start = sprite.src_offset + y * sprite_width * 4;
            let src_row_end = src_row_start + sprite_width * 4;
            ...
            let dest_row_start = (dest_row * atlas_width + dest_x) * 4;
            let dest_row_end = dest_row_start + sprite_width * 4;
...
mod tests {
    ...
    fn duplicated_test_image(width: u32, height: u32) -> (Vec<u8>, usize) {
        let mut pixels = vec![0u8; (width * height * 4 * 2) as usize];
        ...
        let second_offset = (width * height * 4) as usize;
    ...
    fn identical_sprites_produce_identical_fingerprints() {
        ...
        let result =
            detect_families_and_assemble(&pixels, &sprites, TEST_WIDTH * 2, TEST_HEIGHT, 4);
        assert_eq!(result.fingerprints[0], result.fingerprints[1]);
        assert_eq!(
            result.atlas_buffer.len(),
            (TEST_WIDTH * 2 * TEST_HEIGHT * 4) as usize
        );
    }
    #[test]
    ...
    fn downsample_alpha_channel_is_averaged_like_color_channels() {
        ...
        pixels[4..8].copy_from_slice(&[255, 255, 255, 0]);
    ...
    fn shape_all_255_is_fully_opaque() {
        ...
        assert_eq!(
            classify_alpha_shape(&pixels, 0, 2, 2),
            AlphaShape::FullyOpaque
        );
    }
    #[test]
    ...
    fn shape_batch_matches_per_sprite_indices() {
        ...
        assert_eq!(
            shapes,
            vec![AlphaShape::FullyOpaque, AlphaShape::PunchThrough]
        );
    }
    #[test]
    fn shape_out_of_bounds_geometry_does_not_panic() {
        let pixels = vec![0u8; 4];
        assert_eq!(
            classify_alpha_shape(&pixels, 0, 10, 10),
            AlphaShape::FullyOpaque
        );
    }
    #[test]
